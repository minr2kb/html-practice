...
app.get('/portfolio', (req, res) => {
    ...
    res.send(templete.HTML('home', user, status)+`<div class="more" style="justify-content: center; margin-top:100px; color: rgba(255, 255, 255, 0.8); font-family: inherit;">
...
app.get('/portfolio/page/:pageID', (req, res) => {
    ...
    res.cookie('status', 'none');
    if(ID=="home"){
        res.send(templete.HTML(ID, user, status)+`<div class="more" style="justify-content: center; margin-top:100px; color: rgba(255, 255, 255, 0.8); font-family: inherit;">
        <span>Visitors: ${count_visit}</span></div>`);
    }
    else{
...
app.post('/portfolio/create_process', (req, res) => {
    ...
    var thread = req.body;
    console.log(thread);
    posts.post(sanitizer(thread.title), sanitizer(thread.content), req.session.username);
    res.redirect('/portfolio/page/posts');
})
...
app.post('/portfolio/comment', (req,res) => {
    ...
    var result = posts.comments(req.body.postidx, req.body.author, req.body.content);
    res.send(`${result}`);
})
