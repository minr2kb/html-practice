...
module.exports = {
  list: function (user) {
    ...
    while (i < posts.length) {
      var like = connection.query(
        `SELECT * FROM likes WHERE postidx=${posts[i].idx} AND likeduser ='${user}';`
      );
      if (like.length < 1) {
      ...
      if (posts[i].author == user || user == "minr2kb") {
        templete =
          ...
          `<div class="box_posts" id="${posts[i].idx}">
                    <div class="container_posts" onclick="extend(${
                      posts[i].idx
                    });">    
                        <div class="title_posts">${posts[i].title}</div>
                        <div style="display: flex; align-items: center; width: 23.3rem;">
                            <div style="margin-right:87px;" id="likecount${
                              posts[i].idx
                            }">${posts[i].likecnt}</div>
                            <div style="margin-right:70px;" id="commentcount${
                              posts[i].idx
                            }">${posts[i].commentcnt}</div>
                    ...
                    <div style="margin:20px; font-size: 1rem;">
                        ...
                        <div class="reply_container">
                            <input type="text" placeholder="Comment" id="comment${
                              posts[i].idx
                            }" onkeypress="enter3(${posts[i].idx}, '${user}')">
                            <div class="reply_btn" onclick="sendComment(${
                              posts[i].idx
                            }, '${user}');">reply</div>
                            <img src="${img}" style="width:22px;height:22px;margin:10px;margin-left:15px;" onclick="likes(${
            posts[i].idx
          }, '${user}');" id="like${posts[i].idx}"/>
                            <div class="edit_btn" onclick="post_edit(${
                              posts[i].idx
                            });" style="margin-left:auto; margin-right:10px;">Edit</div>
                            <form action="/portfolio/delete_process" id="deleteform${
                              posts[i].idx
                            }" method="post">
                                <input type="hidden" name="idx" value="${
                                  posts[i].idx
                                }">
                                <div class="del_btn" onclick="post_delete(${
                                  posts[i].idx
                                });" style="margin-left:10px;">Delete</div>
                            </form>
                            <form action="/portfolio/edit" id="editform${
                              posts[i].idx
                            }" method="post">
                                <input type="hidden" name="idx" value="${
                                  posts[i].idx
  ...
  post: function (title, content, user) {
    ...
    console.log(date);
    connection.query(
      `INSERT INTO db.posts (title, author, date, content) VALUES ("${title}","${user}","${date}","${content}");`
    );
  },
  ...
  edit: function (idx, title, content) {
    connection.query(
      `UPDATE posts SET title="${title}", content="${content}" WHERE idx=${idx}`
    );
  },
  ...
  likes: function (idx, user, method) {
    if (method == "up") {
      connection.query(
        `INSERT INTO db.likes (postidx, likeduser) VALUES (${idx}, "${user}")`
      );
      connection.query(
        `UPDATE posts SET likecnt = likecnt+1 WHERE idx=${idx};`
      );
      return "liked";
    } else {
      connection.query(
        `DELETE FROM likes WHERE postidx=${idx} AND likeduser="${user}";`
      );
      connection.query(
        `UPDATE posts SET likecnt = likecnt-1 WHERE idx=${idx};`
      );
  ...
  comments: function (postidx, author, content) {
    connection.query(
      `INSERT INTO db.comments (postidx, author, content) VALUES (${postidx}, "${author}", "${content}");`
    );
    connection.query(
